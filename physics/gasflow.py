...
from hydroflow.physics.math import weighted_median,r200_eff,MpcGyr_kms

def retrieve_candidates(galaxy_snapi,galaxy_snapf,pdata_snapi,kdtree_snapi,pdata_snapf,kdtree_snapf):
    r200=r200_eff(galaxy_snapf)
...
def analyse_gasflow(pdata_snapi,pdata_snapf,radius,dt,Tcut=None):
    ...
    if gasflow_output['inflow-n']>0.:
        ...
        gasflow_output['inflow-Z_median']=weighted_median(Z_snap2[inflow_mask],weights=inflow_mass)

        #infall vel
        arvel=(pdata_snapf['R_rel'].values[inflow_mask]-pdata_snapi['R_rel'].values[inflow_mask])/dt*MpcGyr_kms
        gasflow_output['inflow-arvel_mean']=np.average(arvel,weights=inflow_mass)
        gasflow_output['inflow-arvel_median']=weighted_median(arvel,weights=inflow_mass)
    ...
    if gasflow_output['outflow-n']>0.:
        ...
        gasflow_output['outflow-Z_median']=weighted_median(Z_snap1[outflow_mask],weights=outflow_mass)
        
        #ejection vel
        arvel=(pdata_snapf['R_rel'].values[outflow_mask]-pdata_snapi['R_rel'].values[outflow_mask])/dt*MpcGyr_kms
        gasflow_output['outflow-arvel_mean']=np.average(arvel,weights=outflow_mass)
        gasflow_output['outflow-arvel_median']=weighted_median(arvel,weights=outflow_mass)
    ...
    return pd.Series(gasflow_output)
