...
for coordinate in 'xyz':
    data_pd[f'CentreOfPotential_{coordinate}']=data_pd[f'CentreOfPotential_{coordinate}']*0.6777

data_pd=data_pd.loc[np.logical_and(data_pd.Mass>1e10,data_pd.SubGroupNumber==0),:]
data_pd.reset_index(inplace=True,drop=True)# data_pd['Group_R_Crit200']=data_pd['Group_R_Crit200']*0.6777/1000*(1+data_pd['Redshift'])