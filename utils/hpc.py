...
def submit_gasflow_jobarray(exe,arguments,memory,time,array=None):
    ...
    runscriptfilepath=exe
    jobscriptfilepath=f'{jobfolder}submit-{jobname}.slurm'
    ...
    with open(jobscriptfilepath,"w") as jobfile:
        ...
        jobfile.writelines(f"python {runscriptfilepath} --path {pathcat} --nslice {nslice} --snap {snapf} --depth {depth} --ivol $SLURM_ARRAY_TASK_ID \n")
...
def submit_function(repo,function,arguments,memory,time):
    cwd=os.getcwd()
    run=cwd.split('/')[-1]
